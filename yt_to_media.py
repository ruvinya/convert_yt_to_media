...
# Title
title = tk.Label(window, text="Youtube Media Downloader", font=("Oswald Bold", 31))
title.configure(background='#4a9976')
title.grid(column=0, row=0, sticky=tk.EW, padx=5, columnspan=4)

# Link Entry
link_label = tk.Label(window, text="Youtube Link", font=("Oswald Medium", 20))
...
link = tk.Entry(window, textvariable=yt_link, font=("Consolas", 16))
link.configure(background='#57806e')
link.grid(column=0, row=2, sticky=tk.EW, padx=5, columnspan=4)

# Preferences
preferences = tk.Label(window, text="Preferences", font=("Oswald Medium", 20))
...
option_label = tk.Label(window, text="Option", font=("Oswald Light", 15))
...
res_label = tk.Label(window, text="Resolution", font=("Oswald Light", 15))
...
# Description
text_label = res_label = tk.Label(window, text="Status", font=("Oswald Regular", 15))
text_label.configure(background='#4a9976')
text_label.grid(column=1, row=7, padx=5, sticky=tk.N, columnspan=2)

text = tk.Text(window, wrap='word', height=9, width=22)
text.configure(background='#57806e', state="disabled")
text.grid(column=1, row=6, ipadx=50, pady=5, sticky=tk.S, columnspan=2, rowspan=3)
...
def download_file():
    ...
    if os.path.isfile(title + ".mp4"):
        os.remove(title + ".mp4")
    output.download(output_path=os.getcwd())
...
result_label.grid(column=1, row=10, padx=5, sticky=tk.N, columnspan=2)
